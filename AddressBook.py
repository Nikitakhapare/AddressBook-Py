'''
    @Author: Nikita
    @Date: 2022-04-16 09:30:00
    @Last Modified by: Nikita
    @Last Modified time: 2022-04-17 11:00:00
    @Title :Ability to delate the contact in address book.
'''
...
class AddressBook:
    ...
    def delateContact(self):
        """
        Description:
            First check the input taken from user is in list and then delate the list .
        Parameter:
             no parameter
        Return:
            Returning contact dictionary by delating contact if exist
        """
        checkName=input("Enter a name for which you want to delate contact: ")
        for i in range(AddressBook.contact_length):
            if checkName == AddressBook.contact[i][0]:
                del AddressBook.contact[i]
                print("contact Delated succsesfully. ")
                break
...
if __name__ == '__main__':
    ...
    addBook = AddressBook()
    print(
        'Enter 1. To Add Contacts 2. For display a Contact 3.To edit contacts 4.delateContact 5.To Exit')
    while True:
        ...
        if choice == 1:
            ...
            addBook.editContact(firstName=1, lastName=2, address=3,city= 4, state=5, zip=6, phoneNumber=7, emailId=8)
        elif choice == 4:
            addBook.delateContact()
        elif choice == 5:
            exit()
        else:
